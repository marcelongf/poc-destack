...
const uploadFiles = async (req) => {
  ...
  const urls = Object.values(files).map(
    (f) => {
      var _a;
...
const getRouteFromFilename = (filename) => filename === path__default["default"].sep + "default.json" ? path__default["default"].sep : `${filename.slice(0, -5)}`;
...
const fixPaths = (c, basePath) => {
  ...
  const nameWithFixSeps = nameWithoutBasePath.split(path__default["default"].sep).join("/");
...
const loadAllData = async () => {
  ...
  const data = await Promise.all(
    files.map(
      (f) => fs__default["default"].promises.readFile(f, "utf8").then((c) => ({ name: f, content: c })).then((c) => fixPaths(c, basePath))
    )
  );
  return data;
};
const updateData = async (route, data) => {
  const fileName = getFileNameFromRoute(route);
  const updatePath = path__default["default"].join(rootPath, dataFolder);
  const updateFolderExists = await exists(updatePath);
  if (!updateFolderExists) {
    await fs__default["default"].promises.mkdir(updatePath);
  }
  await fs__default["default"].promises.writeFile(path__default["default"].join(updatePath, fileName), JSON.stringify(data));
};
...
const getPackagePath = () => {
  const pathCurrent = path__default["default"].dirname(require.resolve("destack/package.json"));
  if (pathCurrent == null ? void 0 : pathCurrent.startsWith("(api)")) {
    return path__default["default"].join(process.cwd(), "..", pathCurrent);
  } else {
    return pathCurrent;
  }
};
const handleAsset = async (req, res) => {
  ...
  const assetPath = path__default["default"].join(getPackagePath(), req.query.path);
...
const handleTheme = async (req, res) => {
  ...
  const folderPath = path__default["default"].join(getPackagePath(), "themes", themeName);
  const componentNames = await fs__default["default"].promises.readdir(folderPath).then((f) => f.filter((c) => c !== "index.ts"));
  const componentsP = componentNames.map(async (c) => {
    const assetPath = path__default["default"].join(folderPath, c, "index.html");
    const source = await fs__default["default"].promises.readFile(assetPath, "utf-8");
